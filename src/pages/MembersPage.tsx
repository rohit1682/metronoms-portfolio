...
import { randomIndex, shuffled } from '../utils/random';
...
// A tiny rotation+offset per card so the stack looks like a shuffled deck
const STACK_OFFSETS = [
  { rotate: -14, x: -18, y: 0  },
  { rotate:   8, x:  12, y:  6 },
  { rotate: -4,  x:  -6, y: 14 },
...
function CascadeIntro({
  ...
  useEffect(() => {
    // All cards land by: stagger * N + fall_duration
    const landAt = photos.length * 60 + 220; // ~640ms for 7 photos
    const pauseMs = 320;
    const scatterMs = 480;
  ...
  return (
    ...
    >
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: scattering ? 0 : 0.35 }}
      ...
      {/* Falling photo cards */}
      {photos.map((src, i) => {
        const off = STACK_OFFSETS[i % STACK_OFFSETS.length];
        const scatter = SCATTER_DIRECTIONS[i % SCATTER_DIRECTIONS.length];
        return (
          <motion.div
            key={i}
            initial={{
              y: -(globalThis.innerHeight ?? 900) - 200,
              x: off.x * 3,
              rotate: off.rotate * 2.5,
              opacity: 0,
            }}
            animate={scattering
              ? {
            ...
            transition={scattering
              ? { duration: 0.45, ease: [0.4, 0, 0.8, 1] }
              : {
                  duration: 0.22,
                  delay: i * 0.06,
                  ease: [0.22, 1, 0.36, 1],
            ...
            style={{
              ...
              borderRadius: '6px',
              overflow: 'hidden',
              boxShadow: '0 24px 60px rgba(0,0,0,0.85), 0 4px 16px rgba(196,30,58,0.15)',
...
function MemberRow({
  member,
  index,
  // The pool of already-used photo URLs so we don't show the same picture twice
  usedPhotos,
  onPhotoUsed,
}: {
  ...
  // Shuffle once per mount; pick the first photo not already used on the page
...
export default function MembersPage() {
  ...
  // Track which photo URLs are already "featured" on the page to avoid repetition
  const usedPhotoSet = useMemo(() => new Set<string>(), []);
  const onPhotoUsed = useCallback((url: string) => { usedPhotoSet.add(url); }, [usedPhotoSet]);

  // Pick one representative photo per member for the cascade (first from shuffled pool)
  const cascadePhotos = useMemo(() =>
    MEMBERS.list.map(m => {
      const pool = MEMBER_PHOTOS[m.name] ?? [];
  ...
  return (
    <>
      ...
      {/* ── Page content (mounts immediately; fades in after intro) ── */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: introDone ? 1 : 0 }}
        transition={{ duration: 0.5 }}
        style={{ background: '#0a0a0a', minHeight: '100vh', padding: 'clamp(60px, 10vw, 100px) clamp(20px, 5vw, 72px)' }}
      >
